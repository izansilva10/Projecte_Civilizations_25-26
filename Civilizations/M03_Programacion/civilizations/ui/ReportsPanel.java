...
public class ReportsPanel {
    ...
    public ReportsPanel(ArrayList<Battle> battleHistory, Civilization civilization) {
        ...
        this.winsLabel = new Label("0");
        this.lossesLabel = new Label("0");
        this.battleCounterLabel = new Label("0");
    }
    ...
    private void displayBattleData(Battle battle, int battleNum) {
        ...
        // --- COSTES Y PÉRDIDAS FORMATEADOS ---
        HBox costsBox = new HBox(20);
        costsBox.setAlignment(Pos.CENTER);
        costsBox.getChildren().addAll(
            createInfoBox("COSTES INICIALES", formatResourceString(civCost), "#4ea5d9"),
            createInfoBox("COSTES INICIALES", formatResourceString(eneCost), "#e74c3c")
        );
        ...
        lossesBox.getChildren().addAll(
            createInfoBox("PÉRDIDAS", formatResourceString(civLoss), "#4ea5d9"),
            createInfoBox("PÉRDIDAS", formatResourceString(eneLoss), "#e74c3c")
        );
    ...
    private String formatResourceString(String raw) {
        if (raw == null || raw.isEmpty()) return "Sin datos";
        String[] parts = raw.split("\\s+");
        StringBuilder formatted = new StringBuilder();
        for (int i = 0; i < parts.length; i += 2) {
            if (i + 1 < parts.length) {
                formatted.append(parts[i]).append(": ").append(parts[i+1]).append("\n");
            }
        }
